...
import { invoicesApi, manifestsApi, quotationsApi } from '../api/services';
import { errorMessage } from '../api/client';
import { useAuth } from '../features/auth/AuthContext';
import { DeliveryManifestSheet, InvoiceSheet, QuotationSheet } from '../components/tms/TransportPrintLayouts';
...
export default function TmsPrintPage({ resource }) {
  ...
  const service = resource === 'manifests' ? manifestsApi : resource === 'invoices' ? invoicesApi : quotationsApi;
  ...
  const title = resource === 'manifests' ? record.manifestNumber : resource === 'invoices' ? record.invoiceNumber : record.quotationNumber;
  const base = `/${user.role.toLowerCase()}`;
  return <>
    <PageHeader title={title} description={resource === 'manifests' ? 'Delivery manifest format' : resource === 'invoices' ? 'Credit client tax invoice' : 'CRL rate quotation'}>
